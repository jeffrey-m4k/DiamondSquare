...
def main(mapSize):
    ...
    noiseMap[0, 0], noiseMap[0, mapSize - 1], noiseMap[mapSize - 1, 0], noiseMap[
        mapSize - 1, mapSize - 1
    ...
    while stepSize > 1:
        ...
        sys.stdout.flush()
        sys.stdout.write(
            "\rPerforming diamond step [" + str(stepCount) + "/" + stepsRequired + "]"
        )
        ...
        sys.stdout.flush()
        sys.stdout.write(
            "\rPerforming square step  [" + str(stepCount) + "/" + stepsRequired + "]"
        )
...
def getDiamonds(aSize, div, squares):
    ...
    for x in range(len(squares)):
        ...
        squareVList.append(
            [squares[x][0] + (aSize // div), squares[x][1] + (aSize // div)]
        )
...
def diaStep(x, y, stepSize, r):
    ...
    avgValue = avg(
        noiseMap[x, y],
        noiseMap[x + stepSize, y],
        noiseMap[x, y + stepSize],
        noiseMap[x + stepSize, y + stepSize],
    )
...
def squareStep(x, y, stepSize, r, aSize):
    ...
    for coord in range(len(diaCornersFiltered)):
        avgTotal += noiseMap[
            int(diaCornersFiltered[coord][0]), int(diaCornersFiltered[coord][1])
        ]
